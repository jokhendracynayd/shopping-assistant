...
def node_classify(state: ShoppingState) -> dict[str, Any]:
    ...
    try:
        ...
        error_msg = f"LLM intent classification failed: {type(e).__name__}: {e!s}"
        ...
        # This will never execute due to the return above, but satisfies the linter
        pass
...
def _filter_relevant_context(
    question: str, contexts: list[str], min_relevance_threshold: float = 0.3
) -> list[str]:
    """Filter contexts based on relevance and quality."""
    # Constants for text filtering
    MIN_TEXT_LENGTH = 20
    ...
    for text in contexts:
        if not text or len(text.strip()) < MIN_TEXT_LENGTH:  # Skip very short contexts
            continue
        ...
        for existing in filtered_contexts:
            ...
            if len(text) > DUPLICATE_CHECK_LENGTH and text[:DUPLICATE_CHECK_LENGTH] in existing:
                is_duplicate = True
                break
    ...
    # Limit to top contexts for better focus
...
def node_retrieve(state: ShoppingState) -> dict[str, Any]:
    ...
    try:
        ...
        retrieval_quality = (
            "high"
            if len(filtered_contexts) >= HIGH_QUALITY_THRESHOLD
            else "low" if filtered_contexts else "none"
        )

        logger.info(
            f"Retrieved {len(filtered_contexts)} relevant information pieces "
...
def _validate_answer_quality(
    ...
    has_specifics = any(
        [
            ...
            any(
                word in answer_lower for word in ["can", "will", "offer", "feature", "include"]
            ),  # Action words
        ]
    )
...
def node_answer_faq(state: ShoppingState) -> dict[str, Any]:
    ...
    if not context or context == "No relevant information found.":
        ...
        return {
            "answer": (
                "I don't have enough information to answer your question. Please try rephrasing or "
                "asking about our specific products."
            ),
            "confidence": "none",
    ...
    try:
        ...
        confidence = (
            "high"
            if quality_metrics["quality_score"] >= HIGH_QUALITY_THRESHOLD
            else "medium" if quality_metrics["quality_score"] >= MEDIUM_QUALITY_THRESHOLD else "low"
        )
        ...
        if not is_valid:
            ...
            fallback_answer = (
                "I found some information but cannot provide a confident answer about your question. "
                "The available information may not be sufficient or directly related to what you're asking about."
            )
        ...
        logger.info(
            f"Generated answer with {confidence} confidence (score: {quality_metrics['quality_score']})"
        )
        ...
        return {
            "answer": (
                "I'm experiencing technical difficulties while processing your question. "
...
def node_greeting(state: ShoppingState) -> dict[str, Any]:
    ...
    try:
        ...
        return {"answer": answer.strip(), "confidence": "high"}

    except Exception:
        logger.exception("Greeting generation failed")
        return {
            "answer": (
                "Hello! Welcome to our store! I'm so excited to help you find amazing products today. "
...
def node_sales(state: ShoppingState) -> dict[str, Any]:
    ...
    try:
        ...
        logger.exception("Sales response generation failed")
        return {
            "answer": (
                "I'm so excited to help you find the perfect products! We have incredible deals and "
                "top-quality items. What are you looking for today? I'd love to help you find exactly "
                "what you need!"
            ),
            "confidence": "high",
        }
...
def node_product_inquiry(state: ShoppingState) -> dict[str, Any]:
    ...
    try:
        ...
        return {"answer": answer.strip(), "confidence": "high" if context else "medium"}

    except Exception:
        logger.exception("Product inquiry response failed")
        return {
            "answer": (
                "I'd love to help you with that product question! Let me get you the information you need. "
                "Could you tell me more about what specific product or feature you're interested in?"
            ),
            "confidence": "medium",
        }


def node_answer_other(_state: ShoppingState) -> dict[str, Any]:
    """Handle other inquiries with helpful, sales-focused guidance."""
    return {
        "answer": (
            "I'm here to help you with all your shopping needs! I can assist you with finding products, "
            "answering questions about features and specifications, helping with purchases, or providing "
            "information about our policies. What would you like to know about our amazing products?"
        ),
        "confidence": "medium",
    }
...
# Streaming versions of the answer nodes
async def node_answer_faq_stream(state: ShoppingState) -> AsyncIterator[dict[str, Any]]:
    """Stream FAQ answering with real-time response chunks."""
    ...
    # Early return if no context available
    if not context or context == "No relevant information found.":
        logger.info("No context available for streaming answer")
        yield {
            "chunk_type": "metadata",
            "intent": "FAQ",
            "context_count": 0,
        ...
        yield {"chunk_type": "final", "confidence": "none", "quality_metrics": {"no_context": True}}
        return
    ...
    try:
        ...
        # Generate streaming answer using enhanced RAG prompt

        # Get the prompt result first
        prompt_result = rag_prompt.format(question=question, context=context)

        full_answer = ""
        async for chunk in llm_client.astream(prompt_result):
            if chunk:
                full_answer += chunk
                yield {"chunk_type": "content", "content": chunk, "is_final": False}

        # Validate final answer quality
        is_valid, validation_reason, quality_metrics = _validate_answer_quality(
            full_answer, question, context
        )
        ...
        logger.info(
            f"Streamed answer with {confidence} confidence "
        ...
        yield {
            "chunk_type": "error",
            "error": f"Failed to generate streaming response: {e}",
            "fallback": (
                "I'm experiencing technical difficulties while processing your question. "
                "Please try again in a moment."
            ),
...
async def node_greeting_stream(state: ShoppingState) -> AsyncIterator[dict[str, Any]]:
    """Stream greeting responses."""

    question = state["question"]

    try:
        yield {"chunk_type": "metadata", "intent": "Greeting"}

        prompt_result = greeting_prompt.format(question=question)

        async for chunk in llm_client.astream(prompt_result):
            if chunk:
                yield {"chunk_type": "content", "content": chunk, "is_final": False}

        yield {"chunk_type": "final", "confidence": "high"}

    except Exception as e:
        logger.exception("Streaming greeting generation failed")
        yield {
            "chunk_type": "error",
            "error": f"Failed to generate greeting: {e}",
            "fallback": (
                "Hello! Welcome to our store! I'm so excited to help you find amazing products today. "
                "What can I help you discover?"
            ),
...
async def node_sales_stream(state: ShoppingState) -> AsyncIterator[dict[str, Any]]:
    """Stream sales responses."""

    question = state["question"]

    try:
        yield {"chunk_type": "metadata", "intent": "Sales"}

        context = state.get("context", "We have amazing products and great deals available!")
        prompt_result = sales_prompt.format(question=question, context=context)
        ...
        logger.exception("Streaming sales response failed")
        yield {
            "chunk_type": "error",
            "error": f"Failed to generate sales response: {e}",
            "fallback": (
                "I'm so excited to help you find the perfect products! We have incredible deals and "
                "top-quality items. What are you looking for today? I'd love to help you find exactly "
                "what you need!"
...
async def node_product_inquiry_stream(state: ShoppingState) -> AsyncIterator[dict[str, Any]]:
    """Stream product inquiry responses."""

    question = state["question"]
    context = state.get("context", "")

    try:
        yield {"chunk_type": "metadata", "intent": "Product_Inquiry", "has_context": bool(context)}

        prompt_result = product_inquiry_prompt.format(question=question, context=context)

        async for chunk in llm_client.astream(prompt_result):
            if chunk:
                yield {"chunk_type": "content", "content": chunk, "is_final": False}

        yield {"chunk_type": "final", "confidence": "high" if context else "medium"}

    except Exception as e:
        logger.exception("Streaming product inquiry response failed")
        yield {
            "chunk_type": "error",
            "error": f"Failed to generate product response: {e}",
            "fallback": (
                "I'd love to help you with that product question! Let me get you the information you need. "
                "Could you tell me more about what specific product or feature you're interested in?"
            ),
...
async def _handle_streaming_intent(
    intent: str, streaming_state: ShoppingState
) -> AsyncIterator[dict[str, Any]]:
    """Handle streaming for different intent types."""
    if intent.lower() == "greeting":
        async for chunk in node_greeting_stream(streaming_state):
            yield chunk
    elif intent.lower() == "sales":
        async for chunk in node_sales_stream(streaming_state):
            yield chunk
    elif intent.lower() == "product_inquiry":
        async for chunk in node_product_inquiry_stream(streaming_state):
            yield chunk
    elif intent.lower() == "faq":
        async for chunk in node_answer_faq_stream(streaming_state):
            yield chunk
    else:
        # Handle "Other" intent
        yield {"chunk_type": "metadata", "intent": "Other"}
        yield {
            "chunk_type": "content",
            "content": (
                "I'm here to help you with all your shopping needs! I can assist you with finding products, "
        ...
        yield {"chunk_type": "final", "confidence": "medium"}


async def run_shopping_graph_stream(question: str) -> AsyncIterator[dict[str, Any]]:
    """Execute the shopping graph with streaming responses."""

    try:
        # First, run the classification and context retrieval (non-streaming parts)
        initial: ShoppingState = {"question": question}

        # Classify intent
        intent_result = node_classify(initial)
        intent = intent_result.get("intent", "Other")

        # Send intent information first
        yield {"chunk_type": "intent", "intent": intent, "question": question}

        # Determine if we need context retrieval
        needs_context = intent.lower() in ["faq", "product_inquiry"]
        context_result = {}

        if needs_context:
            # Retrieve context
            yield {"chunk_type": "retrieving", "message": "Searching for relevant information..."}

            context_result = node_retrieve(initial)

            yield {
                "chunk_type": "context_retrieved",
                "context_count": context_result.get("context_count", 0),
                "retrieval_quality": context_result.get("retrieval_quality", "unknown"),
            }

        # Prepare state for streaming
        streaming_state = {"question": question, "intent": intent, **context_result}

        # Route to appropriate streaming handler
        async for chunk in _handle_streaming_intent(intent, streaming_state):
            yield chunk

        # Send completion signal
        yield {"chunk_type": "complete"}

    except Exception as e:
        logger.exception("Error in streaming graph execution")
        yield {
            "chunk_type": "error",
            "error": str(e),
